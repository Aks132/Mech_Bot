  ...
  if (enable_encoder == false) 
    ...
    if (enable_encoder == false) 
      ...
      if (B_position_error[0] > 10) {
        ...
        if(millis() - error_starttime > 500 && temp_error ==  Current_brake_position[0])
        {
          if(Brake_Speed < 50){
            Current_brake_position[0] = 0;
            B_Current_position_B[0]=0;
            B_Current_position_A[0]=0;
            error_starttime = millis();
            }
        
       
        }
        else if(temp_error != Current_brake_position[0])
        {
        error_starttime = millis();
          }
        temp_error = Current_brake_position[0];
;
