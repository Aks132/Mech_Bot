void drive_mode(){
  ...
  else if(Drive_mode == 'N'){
        digitalWrite(Neutral_pin,LOW);
            digitalWrite(Reverse_pin,HIGH);


  }
  else if(Drive_mode == 'R') {
...
for(int h =0; h<=1; h++){
     pinMode(Head_light_pin[h], OUTPUT);
   pinMode(back_light_pin[h], OUTPUT);
   pinMode(side_light_pin[h], OUTPUT);

}
  digitalWrite(Head_light_pin[0],HIGH);
   digitalWrite(Head_light_pin[1],HIGH);
...
if(left_side_light == 'F'){
  ...
  if(millis() - side_light_start > 2000 && side_light_state == true){
  digitalWrite(side_light_pin[0],LOW);
   side_light_state = !side_light_state;
   side_light_start = millis();

  }
  else if(millis() - side_light_start > 2000 && side_light_state == false){
    side_light_start = millis();
    side_light_state = !side_light_state;
    digitalWrite(side_light_pin[0],HIGH);
    }
...
//-----------Head Light --------------------
if(Head_light_mode == 'D'){
        digitalWrite(Head_light_pin[0],HIGH);
        digitalWrite(Head_light_pin[1],LOW);

  
}
else if(Head_light_mode == 'U'){
        digitalWrite(Head_light_pin[0],LOW);
        digitalWrite(Head_light_pin[1],LOW);  
}
else if(Head_light_mode == 'F'){
          digitalWrite(Head_light_pin[0],HIGH);
        digitalWrite(Head_light_pin[1],HIGH);
}
else{
